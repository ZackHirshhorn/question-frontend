...
describe('AnswerPage linking flow (authenticated)', () => {
  ...
  it('creates, links via auth PUT, then redirects to answer page', async () => {
    ...
    vi.spyOn(questionnaireApi, 'createQuestionnaire').mockResolvedValue({ data: created } as any);
    const authSpy = vi.spyOn(questionnaireApi, 'updateQuestionnaireAnswersAuth').mockResolvedValue({ data: { ok: true } } as any);
