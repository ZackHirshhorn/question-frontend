...
const AnswerPage: React.FC = () => {
  ...
  const createAndRedirect = async (payload: { name?: string; email?: string; phone?: string }) => {
    ...
    try {
      ...
      const questionSummaries: Array<{ path: string; count: number; firstQuestion?: unknown }> = [];
      const scanForQuestionCols = (value: unknown, path: string[]) => {
        if (!value || typeof value !== 'object') {
          return;
        }
        if (Array.isArray(value)) {
          value.forEach((item, index) => {
            scanForQuestionCols(item, [...path, `[${index}]`]);
          });
          return;
        }
        Object.entries(value as Record<string, unknown>).forEach(([key, nested]) => {
          const nextPath = [...path, key];
          if (key === 'questions' && Array.isArray(nested) && nested.length > 0) {
            const firstQuestion = nested[0] && typeof nested[0] === 'object' ? nested[0] : undefined;
          ...
          scanForQuestionCols(nested, nextPath);
        });
      };
      scanForQuestionCols(nextQuestionnaire, []);
      // eslint-disable-next-line no-console
      console.log('[DEBUG] questionnaire question fields:', questionSummaries.length ? questionSummaries : 'none');
      ...
      if (qid) {
        ...
        // Inform the user that creation currently requires two API calls
        try {
          window.alert('Note: questionnaire creation currently uses two API calls (create + update).');
        } catch {
