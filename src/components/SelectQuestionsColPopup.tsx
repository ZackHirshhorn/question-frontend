...
const SelectQuestionsColPopup: React.FC<SelectQuestionsColPopupProps> = ({ title = 'בחירת אסופת שאלות', onClose, onSelect, initialSelectedId }) => {
  ...
  // Keep local selected state in sync if parent provides a new initial id
  useEffect(() => {
  ...
  const selected = collections.find((c) => c._id === selectedId);
              ...
              collections.map((col) => (
                ...
                >
                  <input
                    ...
                    name="collection"
                    value={col._id}
                    checked={selectedId === col._id}
                    onChange={() => setSelectedId(col._id)}
                  />
